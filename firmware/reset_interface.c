#include <stdbool.h>
#include <stdint.h>

#include "device/usbd_pvt.h"
#include "hardware/watchdog.h"
#include "pico/bootrom.h"
#include "pico/usb_reset_interface.h"
#include "tusb.h"

#ifndef MAGICTOOL_RESET_TO_FLASH_DELAY_MS
#define MAGICTOOL_RESET_TO_FLASH_DELAY_MS 100
#endif

static uint8_t g_reset_interface_number;

static void resetd_init(void) {
}

static void resetd_reset(uint8_t rhport) {
    (void)rhport;
    g_reset_interface_number = 0;
}

static uint16_t resetd_open(uint8_t rhport, tusb_desc_interface_t const *itf_desc, uint16_t max_len) {
    (void)rhport;

    TU_VERIFY(itf_desc->bInterfaceClass == TUSB_CLASS_VENDOR_SPECIFIC &&
                  itf_desc->bInterfaceSubClass == RESET_INTERFACE_SUBCLASS &&
                  itf_desc->bInterfaceProtocol == RESET_INTERFACE_PROTOCOL,
              0);
    TU_VERIFY(max_len >= sizeof(tusb_desc_interface_t), 0);

    g_reset_interface_number = itf_desc->bInterfaceNumber;
    return sizeof(tusb_desc_interface_t);
}

static bool resetd_control_xfer_cb(uint8_t rhport, uint8_t stage, tusb_control_request_t const *request) {
    (void)rhport;

    if (stage != CONTROL_STAGE_SETUP) {
        return true;
    }

    if (request->wIndex != g_reset_interface_number) {
        return false;
    }

    if (request->bRequest == RESET_REQUEST_BOOTSEL) {
        const uint32_t disable_mask = request->wValue & 0x7fu;
        rom_reset_usb_boot_extra(-1, disable_mask, false);
    }

    if (request->bRequest == RESET_REQUEST_FLASH) {
        watchdog_reboot(0, 0, MAGICTOOL_RESET_TO_FLASH_DELAY_MS);
        return true;
    }

    return false;
}

static bool resetd_xfer_cb(uint8_t rhport, uint8_t ep_addr, xfer_result_t result, uint32_t xferred_bytes) {
    (void)rhport;
    (void)ep_addr;
    (void)result;
    (void)xferred_bytes;
    return true;
}

static usbd_class_driver_t const kResetDriver = {
#if CFG_TUSB_DEBUG >= 2
    .name = "RESET",
#endif
    .init = resetd_init,
    .reset = resetd_reset,
    .open = resetd_open,
    .control_xfer_cb = resetd_control_xfer_cb,
    .xfer_cb = resetd_xfer_cb,
    .sof = NULL,
};

usbd_class_driver_t const *usbd_app_driver_get_cb(uint8_t *driver_count) {
    *driver_count = 1;
    return &kResetDriver;
}
